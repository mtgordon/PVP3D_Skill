"""Lightweight FEBio .feb parser for structural diagnostics (no FEBio run needed).

Parses <Mesh> into plain dicts: nodes {id: xyz}, element blocks {name: (type, {eid: [nids]})},
node sets, surfaces (facet lists), surface pairs, discrete sets. Also provides an exact
point-to-triangle distance and a point-to-surface query (quads split into two triangles).

Mapping by coordinate: converted/re-exported meshes often keep orphan nodes at the *same*
coordinates as live ones (old copies of a part, remesh leftovers). Restrict nearest-node
searches to nodes that are actually used by the elements you care about
(`domain_nodes(name)` / `nodes_in_elements()`), or they will happily return a dead node.

Usage as a library:  from feb_model import Feb, dist_to_surface
